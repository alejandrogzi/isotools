...
def process_chunk(
    args: argparse.Namespace, model: str, encoder: str
...
def write_results(bedgraph: List[str], bed_lines: List[str], path: str) -> None:
    ...
    bg = f"polya_{suffix}.bedGraph"
    bed = f"polya_{suffix}.bed"
...
def run_apparent(model: str, encoder: str, seq: str) -> Tuple[List[int], List[float]]:
    """
    Run APPARENT to estimate poly(A) tail length from a chunked file
